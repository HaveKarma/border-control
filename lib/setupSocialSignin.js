...
var _setupStrategy = function(app, Strategy, options) {
    ...
    passport.use(options.siteName+'-karma-auth', new Strategy(options.credentials,
        function(req, accessToken, refreshToken, profile, done) {
            console.log('\n\n\n\nDONE: '.blue.inverse + JSON.stringify(done));
            ...
            LoginUtils.processSuccessfulAuthorization(req, options.siteName, profile, done, key);
...
module.exports = function(app, options) {
    ...
    // step to "options"
    var packageName = options.packageName || 'passport-'+ options.siteName;
